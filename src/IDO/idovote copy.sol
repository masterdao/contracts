...
contract idovoteContract is Ownable {
    ...
    function vote(address coinAddress, bool bStatus) public returns (bool) {
        ...
        voteVeDao = voteVeDao.add(
            votePeople[msg.sender][coinAddress].veDao.mul(votePeople[msg.sender][coinAddress].weight).div(10)
        );
    ...
    function vote_old(address coinAddress, bool bStatus) public returns (bool) {
        ...
        if (lastVoteAddress[msg.sender] == address(0)) {
            ...
            if (isOk) {
                ...
                //结算投票后收益
                uint256 peopleVoteIncome = votePeople[msg.sender][coinAddress].veDao.mul(
    ...
    //管理员设定投票分配收益
    function setDaoVoteIncome(address coinAddress, uint256 amount) public payable onlyOwner returns (address, uint256) {
