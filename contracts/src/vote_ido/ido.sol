...
contract idoCoinContract is Ownable {
    ...
    function calculateMakeCoinAmount(address coinAddress, uint256 winningRate)
        private
        view
        returns (uint256)
    {
        ...
        makeCoinAmount = makeCoinAmount.mul(
    ...
    function calculateTakeBalnce(address coinAddress, uint256 winningRate)
        private
        view
        returns (uint256)
    {
        uint256 takeBalance = usercoin[msg.sender][coinAddress]
            .takeCoinAmount
            .mul(winningRate)
            .div(1e10);
        return takeBalance;
    ...
    function withdraw(address coinAddress, uint256 winningRate)
        public
        returns (bool)
    {
        ...
        uint256 makeCoinAmount = calculateMakeCoinAmount(
            coinAddress,
            winningRate
        ); //计算用于可以购买多少币
    ...
    function takeOut(address coinAddress) public returns (bool) {
        ...
        if (idoCoin[coinAddress].idoCoinHead.bDAO) {
            collectAmount = collectAmount.sub(
                idoCoin[coinAddress].daoCollectAmount
            );
        ...
        uint256 balance = (idoCoin[coinAddress].idoCoinHead.idoAmount).sub(
        ...
        // dexCoinAmount = dexCoinAmount.mul(
        // idoCoin[coinAddress].idoCoinHead.price
        // );
