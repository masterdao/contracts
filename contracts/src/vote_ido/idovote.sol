...
contract idovoteContract is Ownable {
    ...
    //设定通过率
    function setpassingRate(uint256 _passingRate)
        public
        onlyOwner
        returns (uint256)
    {
    ...
    //设定投票率
    function setvotingRatio(uint256 _votingRatio)
        public
        onlyOwner
        returns (uint256)
    {
    ...
    //获取币投票信息
    function getvotecoin(address coinAddress)
        public
        view
        returns (vcoinInfo memory)
    {
    ...
    function vote(
        ...
        require(coinAddress != address(0));
        require(
            daoMintingPool.getminerVeDao(msg.sender, coinAddress, poolTypeId) >
                0
        );
        ...
        peopleInfo memory newpeopleInfo = peopleInfo({
            ...
            veDao: daoMintingPool.getminerVeDao(
                msg.sender,
                coinAddress,
                poolTypeId
            ),
            ...
            peopleVoteIncome: votePeople[msg.sender][coinAddress]
                .peopleVoteIncome,
            bStatus: bStatus
        });
        ...
        if (lastVoteAddress[msg.sender] == address(0)) {
            ...
            isOk = lastVoteStatus[msg.sender] ==
                votecoin[lastVoteAddress[msg.sender]].bSuccessOrFail
                ? true
                : false;
            if (isOk) {
                votePeople[msg.sender][coinAddress].weight = (
                    votePeople[msg.sender][coinAddress].weight
                ).add(1);
                //结算投票后收益
                uint256 peopleVoteIncome = votePeople[msg.sender][coinAddress]
                    .veDao
                    .mul(votePeople[msg.sender][coinAddress].weight);
                peopleVoteIncome = peopleVoteIncome.div(10);

                if (lastVoteStatus[msg.sender]) {
                    peopleVoteIncome = peopleVoteIncome.div(
                        votecoin[coinAddress].pass
                    );
                } else {
                    peopleVoteIncome = peopleVoteIncome.div(
                        votecoin[coinAddress].deny
                    );
                }
                votePeople[msg.sender][coinAddress]
                    .peopleVoteIncome = peopleVoteIncome;
                //累计用户总投票收益
                peopleTotalVoteIncome[msg.sender] = peopleTotalVoteIncome[
                    msg.sender
                ].add(peopleVoteIncome);
            } else {
                votePeople[msg.sender][coinAddress].weight = (
                    votePeople[msg.sender][coinAddress].weight
                ).sub(1);
        ...
        if (bStatus) {
            votecoin[coinAddress].pass = votecoin[coinAddress].pass.add(
                voteVeDao.mul(weight).div(10)
            );
        } else {
            votecoin[coinAddress].deny = votecoin[coinAddress].deny.add(
                voteVeDao.mul(weight).div(10)
            );
        }
        totalStaking = daoMintingPool.getcalculatestakingAmount();
        votecoin[coinAddress].cpassingRate = votecoin[coinAddress]
            .pass
            .mul(100)
            .div(votecoin[coinAddress].pass.add(votecoin[coinAddress].deny));
        votecoin[coinAddress].cvotingRatio = votecoin[coinAddress]
            .voteVeDao
            .mul(100)
            .div(totalStaking);
    ...
    //管理员设定否票结束
    function setVoteCoinEnd(address coinAddress)
        public
        onlyOwner
        returns (bool)
    {
        ...
        if (
            votecoin[coinAddress].cpassingRate >= passingRate &&
            votecoin[coinAddress].cvotingRatio >= votingRatio
        ) {
